...
impl SolverService for RServer {
    async fn solve(&self,request:Request<SolveRequest>)->Result<Response<SolveResponse>,Status>{
        ...
        if rq.year == 2023 && rq.day == 20 && rq.part == 1 {
            let tanswer = day20::solve_day20_part1(rq.data);
            println!("Returning {}", tanswer);
            return Ok(Response::new(SolveResponse{
                string_answer:"".to_string(),
                big_answer:tanswer,
        ...
        if rq.year == 2023 && rq.day == 22 && rq.part == 1 {
            let tanswer = day22::solve_day22_part1(rq.data);
            println!("Returning {}", tanswer);
            return Ok(Response::new(SolveResponse{
                string_answer:"".to_string(),
                big_answer:0,
        ...
        if rq.year == 2023 && rq.day == 25 && rq.part == 1 {
            let tanswer = day25::solve_day25_part1(rq.data);
            println!("Returning {}", tanswer);
            return Ok(Response::new(SolveResponse{
                string_answer:"".to_string(),
                big_answer:0,
